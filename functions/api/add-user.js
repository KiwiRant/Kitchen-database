...
export async function onRequestPost({ request, env }) {
  ...
  const insertColumns = [metadata.identifierColumn, metadata.passwordColumn];
  ...
  if (metadata.columns.role) {
    ...
    if (roleValue) {
      insertColumns.push(metadata.columns.role.name);
  ...
  try {
    ...
    return jsonResponse({
      ...
      user: {
        ...
        role: roleColumnName && insertColumns.includes(roleColumnName)
          ? insertValues[insertColumns.indexOf(roleColumnName)]
          : undefined,
      },
    });
  } catch (error) {
