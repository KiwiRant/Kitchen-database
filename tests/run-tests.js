...
import { getUsersMetadata, quoteIdentifier } from '../functions/api/_utils.js';
...
async function testDetectsPwdColumn() {
  const db = new FakeD1Database([
    { name: 'userEmail', notnull: 1, dflt_value: null },
    { name: 'pwd', notnull: 1, dflt_value: null },
  ]);

  const meta = await getUsersMetadata(db);
  assert.equal(meta.identifierColumn, 'userEmail');
  assert.equal(meta.passwordColumn, 'pwd');
}

function testQuotesIdentifier() {
  assert.equal(quoteIdentifier('email'), '"email"');
...
(async () => {
  ...
  await testDetectsPwdColumn();
  testQuotesIdentifier();
